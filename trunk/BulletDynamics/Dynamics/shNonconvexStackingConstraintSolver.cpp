...
shNonconvexStackingConstraintSolver::shNonconvexStackingConstraintSolver()
{
...
void shNonconvexStackingConstraintSolver::reset()
{
	CollisionHappened = 0.f;
...
btScalar shNonconvexStackingConstraintSolver::solveGroup( btCollisionObject** bodies, int numBodies, btPersistentManifold** manifoldPtr, int numManifolds, btTypedConstraint** constraints, int numConstraints, const btContactSolverInfo& infoGlobal, btIDebugDraw* debugDrawer, btStackAlloc* stackAlloc, btDispatcher* /*dispatcher*/)
{
...
btScalar shNonconvexStackingConstraintSolver::solveCollisionConstraints( btPersistentManifold** manifoldPtr, int numManifolds, const btContactSolverInfo& infoGlobal, btIDebugDraw* debugDrawer, btStackAlloc* stackAlloc )
{
...
//printf( "Num manifolds = %d\n", numManifolds );
	...
	for ( int i = 0; i < numManifolds; i++ )
	{
		...
		btScalar nc = btScalar(numContacts);
		printf( "rBTot before divide = %f %f %f\n", rBTot.x(), rBTot.y(), rBTot.z() );
		printf( "  nc = %f\n", nc );
		rBTot = btVector3( rBTot.x() / nc, rBTot.y() / nc, rBTot.z() / nc );

		/*// Order the contact points in order of depth
		for ( int j = 0; j < numContacts; j++ )
		{
			for ( int k = j + 1; k < numContacts; k++ )
			{
				if ( manifold->getContactPoint( j ).getDistance() < manifold->getContactPoint( k ).getDistance() )
				{
					printf( "*******************SWAPPING*************************\n" );
					manifold->swapContactPoints( j, k );
		...
		do
		{
			...
			foundAndComputedCollision = computeContactForDeepestNondivergingPointOnManifold( manifold, infoGlobal, rBTot );
			
			// If a colliding pair of points was found, this will flag the do-while loop to run through the remaining points in the manifold again
			if ( foundAndComputedCollision ) CollisionHappened = 1.f;
			
			count++;
			
		} while ( /*foundAndComputedCollision == btScalar(1.f) &&*/ manifold->getNumContacts() > 0 );		// repeat until all collision points are diverging (at which point the manifold should be empty of points)
		
		/*// Both solverBody objects update their collisionObjects' velocities
...
btScalar shNonconvexStackingConstraintSolver::computeContactForDeepestNondivergingPointOnManifold( btPersistentManifold* manifold, const btContactSolverInfo& infoGlobal, btVector3& rBTot )
{
	// extract the colliding objects from the manifold
	btCollisionObject* colObjA = (btCollisionObject*)manifold->getBody0();
	btCollisionObject* colObjB = (btCollisionObject*)manifold->getBody1();
	...
	for ( int j = 0; j < numContacts; j++ )
	{
		...
		//printf( "rB asdf = %f %f %f\n", rB.x(), rB.y(), rB.z() );
		//rB = btVector3( 0.0, rB.y(), 0.0 );
...
void shNonconvexStackingConstraintSolver::computeVelocitiesForCollisionPair( btRigidBody* rbA, btRigidBody* rbB, btSolverConstraint& solverConstraint )
{
...
void shNonconvexStackingConstraintSolver::computeImpulseForCollisionPair( btRigidBody* rbA, btRigidBody* rbB, btSolverConstraint& solverConstraint )
{
	btVector3 rA = solverConstraint.m_relPos1;
	btVector3 rB = solverConstraint.m_relPos2;
...
void shNonconvexStackingConstraintSolver::applyImpulseForCollisionPair( btRigidBody* rbA, btRigidBody* rbB, btSolverConstraint& solverConstraint )
...
void shNonconvexStackingConstraintSolver::updateVelocitiesForCollisionPair( int objectIdA, int objectIdB, btRigidBody* rbA, btRigidBody* rbB, btScalar timeStep ) {
...
btScalar shNonconvexStackingConstraintSolver::solveNonContactConstraints( btTypedConstraint** constraints, int numConstraints, const btContactSolverInfo& infoGlobal, btIDebugDraw* debugDrawer, btStackAlloc* stackAlloc )
{
...
void shNonconvexStackingConstraintSolver::initSolverBody( shSolverBody* solverBody, btCollisionObject* collisionObject )
{
...
int	shNonconvexStackingConstraintSolver::getOrInitSolverBody( btCollisionObject& body )
{
	int solverBodyIdA = -1;
...
bool shNonconvexStackingConstraintSolver::thereWasACollision() {
