...
if bulletPath and houdiniPath:
    ...
    bulletDynamicsPath = bulletPath + '\\lib\\Release'
    bulletCollisionPath = bulletPath + '\\lib\\Release'
    linearMathPath = bulletPath + '\\lib\\Release'
    
    simSolverBulletSrcPath = 'J:/Animation/Houdini/toolkit/SIM/SIM_SolverBullet'   #houdiniPath + '\\toolkit\\SIM_SnowBulletSolver\\src'
    #simSolverBulletSrcPath = simSolverBulletSrcPath.replace( '\\', '/' )
    
    bulletSrcPath = bulletPath + '\\src'
    bulletSrcPath = bulletSrcPath.replace( '/', '\\' )
    
    hcustomCommand = 'hcustom -I ' + bulletSrcPath + ' -l BulletDynamics -l BulletCollision -l LinearMath -L ' + bulletDynamicsPath + ' -L ' + bulletCollisionPath + ' -L ' + linearMathPath + ' SIM_SolverBulletHolladay.cpp'
